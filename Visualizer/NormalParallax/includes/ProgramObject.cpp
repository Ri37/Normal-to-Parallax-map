...
ProgramObject::ProgramObject() :
	...
	m_list_shaders_attached(),
	m_map_uniform_locations()
{

}

ProgramObject::~ProgramObject() noexcept
{
	if (m_id != 0)
	{
		Clean();
...
ProgramObject::ProgramObject(ProgramObject&& rhs) noexcept :
	m_id(rhs.m_id),
...
bool ProgramObject::Init(
	std::initializer_list<ShaderObject> shaderList,
	std::initializer_list<Binding> attribLocationBindingList,
	std::initializer_list<Binding> fragDataBindingList)
{
	if (m_id != 0)
		Clean();
...
void ProgramObject::Clean() const
{
...
bool ProgramObject::LinkProgram() const
{
	if (m_id == 0)
		return false;
...
GLint ProgramObject::GLResolveUniformLocation(GLint _uniform) const
{
	return _uniform;
}
